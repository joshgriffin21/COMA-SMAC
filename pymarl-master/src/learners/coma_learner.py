...
from modules.mixers.qmix import QMixer, NonMonotonic
...
class COMALearner:
    ...
    def _train_critic(self, batch, rewards, terminated, actions, avail_actions, mask, bs, max_t):
        ...
        if self.target_mixer is not None:
            targets_taken = self.target_mixer(targets_taken, batch["state"][:, :])
        ...
        for t in reversed(range(rewards.size(1))):
            ...
            if(str(self.mixer) == "VDNMixer()"):  
                ...
                td_error = (q_taken[:, 0] - targets_t.detach())
            else:
                q_t = self.critic(batch, t)
                q_vals[:, t] = q_t.view(bs, self.n_agents, self.n_actions)
                q_taken = th.gather(q_t, dim=3, index=actions[:, t:t+1]).squeeze(3).squeeze(1)
